...
### 나라's 답변: 우선 유저가 페이지의 URL주소를 입력하면 도메인 부분을 DNS서버에서 검색을 합니다. 이때 DNS서버에서는 도메인 이름에 해당하는 IP주소를 찾고 입력한 URL정보와 함께 브라우저로 전달합니다. 이렇게 전달받은 IP주소를 HTTP프로토콜을 이용하여 요청 메세지를 생성하고 이 메세지를 TCP프로토콜에 맞게 인터넷을 거쳐 서버에 전송됩니다. 이 요청 메세지는 서버에 도착했을 때 URL정보로 변환되고 웹 서버는 URL에 맞는 데이터를 검색하고 해당 리소스들을 브라우저에게 HTTP응답 메세지를 생성하고 TCP프로토콜을 거쳐 브라우저에 저송됩니다. 브라우저는 다시 HTTP프로토콜을 사용하여 브라우저 데이터로 변환하여 이 변환된 데이터를 기준으로 브라우저에 렌더링합니다.
